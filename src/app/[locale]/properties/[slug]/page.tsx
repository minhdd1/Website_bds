...
import { ArrowLeft, MapPin, BedDouble, Bath, Maximize2, Sparkles, Check } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-12 animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        <div className="flex items-center text-xs text-clay-accent uppercase tracking-[0.2em] font-semibold">
          <span>{prop.type === "apartment" ? commonT("apartment") : commonT("office")}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          {/* Đánh giá phong cách sống (Lifestyle Match) */}
          <div className="space-y-6 p-8 bg-linen/30 border border-stone/20">
            <h2 className="font-serif text-2xl text-charcoal flex items-center space-x-2">
              ...
              <span>{commonT("lifestyleMatch")}</span>
            </h2>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="border border-stone/30 bg-linen/20 p-6 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between border-b border-stone/20 pb-2">
                <span className="text-charcoal/50">{commonT("price")}</span>
              ...
              <div className="flex justify-between border-b border-stone/20 pb-2">
                <span className="text-charcoal/50">{commonT("bedrooms")}</span>
                <span>{prop.bedrooms ? prop.bedrooms : commonT("notAvailable")}</span>
              </div>
              <div className="flex justify-between border-b border-stone/20 pb-2">
                <span className="text-charcoal/50">{commonT("bathrooms")}</span>
                <span>{prop.bathrooms ? prop.bathrooms : commonT("notAvailable")}</span>
              </div>
              <div className="flex justify-between pb-1">
                <span className="text-charcoal/50">{commonT("furnished")}</span>
                <span className="text-right max-w-[180px] line-clamp-1">
          ...
          <div className="border-l-4 border-clay bg-linen p-6 space-y-4 rounded-r shadow-sm">
            <h3 className="font-serif text-lg text-clay-accent font-semibold italic flex items-center space-x-1">
